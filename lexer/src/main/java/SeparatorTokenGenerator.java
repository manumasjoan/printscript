...
public class SeparatorTokenGenerator implements TokenGenerator {
    ...
    private LexicalRangeState updateState(LexicalRangeState lexicalRangeState) {
        return lexicalRangeState.updateState(lexicalRangeState.getIndex()+1, lexicalRangeState.getLine()+1,0);
    }

    @NotNull
    private Token createToken(LexicalRangeState lexicalRangeState) {
        int index= lexicalRangeState.getIndex();
        Token token= new Token(DefaultTokenTypes.SEPARATOR, index,index,new LexicalRange(lexicalRangeState.getColumn(), lexicalRangeState.getLine(), lexicalRangeState.getColumn(), lexicalRangeState.getLine()));
        return token;
    }
